...
window.addEventListener("load", function () {
  if (!document.body.classList.contains("news-letter-cf")) return;

  const introButton = document.querySelector(".newsletter-watch2");
  const currentVideo = document.querySelector("#main-video-js");
  const movieModalWrapper = document.querySelector(".movie-modal-wrapper");
  const movieModalClose = document.querySelector(".movie-modal-close");
...
jQuery(".slider-nav-news").slick({
  ...
  fade: true,
  arrows: true,
  appendArrows: jQuery(".slider-nav-news")
    .parents(".newsletter-slider")
    .find(".sldr_arr_news"),
  asNavFor: ".slider-for-news",
});

// slider at /newsletter (for)
